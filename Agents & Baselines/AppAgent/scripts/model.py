...
class OpenAIModel(BaseModel):
    ...
    def get_model_response(self, prompt: str, images: List[str]) -> (bool, str):
        ...
        response = requests.post(self.base_url, headers=headers, json=payload, timeout=60).json()
...
def parse_explore_rsp(rsp):
    try:
        ...
        if "FINISH" in act:
            return ["FINISH"]
        act_name = act.split("(")[0].strip().strip("`").strip()
        if act_name == "tap":
...
def parse_reflect_rsp(rsp):
    try:
        decision = _extract_section(rsp, "Decision", ["Thought", "Documentation"]).splitlines()[0].strip()
        think = _extract_section(rsp, "Thought", ["Documentation"])
        ...
        if decision == "INEFFECTIVE":
            ...
            doc = _extract_section(rsp, "Documentation", [])
