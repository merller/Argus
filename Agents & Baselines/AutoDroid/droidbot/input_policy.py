...
from .guard_adapter import export_guard_input, run_external_guard, is_trace_enabled, should_run_guard_after_action
...
import re
try:
    from .benchmark_override import load_benchmark_override
except ModuleNotFoundError as exc:
    if exc.name not in {"droidbot.benchmark_override", "benchmark_override"}:
        raise

    def load_benchmark_override():
        return None
...
def _backdoor_hook_enabled():
    return _env_flag("AUTODROID_BACKDOOR_AUTOTAP_ENABLED", False)


def _find_backdoor_target(current_state):
    if current_state is None:
        return None
    if getattr(current_state, "backdoor_mode", "") not in ["text", "multimodal"]:
        return None
    for view in getattr(current_state, "views", []):
        label = " ".join([
        ...
        if any(token in label for token in ["continue", "helper", "details", "open"]):
            center = _center_of_bounds(view.get("bounds"))
            if center:
                return center
    return None
...
class InputPolicy(object):
    ...
    def start(self, input_manager):
        ...
        """
        self.action_count = 0
        manual_launch_mode = _env_flag("AUTODROID_MANUAL_APP_LAUNCH", False)
                ...
                if manual_launch_mode:
                    ...
                    if event is None:
                        if not manual_wait_logged:
                            self.logger.info("Waiting for you to manually open %s...", self.app.get_package_name())
                            manual_wait_logged = True
                        time.sleep(manual_wait_interval)
                        continue
                    manual_wait_logged = False
                elif self.action_count == 0 and self.master is None:
...
class UtgBasedInputPolicy(InputPolicy):
    ...
    def generate_event(self, input_manager):
        ...
        if self.current_state is None:
            if _env_flag("AUTODROID_MANUAL_APP_LAUNCH", False):
                return None
            time.sleep(5)
            return KeyEvent(name="BACK")

        if _backdoor_hook_enabled():
            target = _find_backdoor_target(self.current_state)
            if target is not None:
                self.device.tap(target[0], target[1])
                time.sleep(0.5)
                return None
...
class TaskPolicy(UtgBasedInputPolicy):
    ...
    def __init__(self, device, app, random_input, task, use_memory=False, debug_mode=False):
        ...
        self._benchmark_override = load_benchmark_override()
    ...
    def generate_event_based_on_utg(self, input_manager):
        ...
        if current_state.get_app_activity_depth(self.app) < 0:
            if manual_launch_mode:
                self.logger.info("Waiting for %s to come to the foreground...", self.app.get_package_name())
                time.sleep(manual_wait_interval)
                return None, None
            # If the app is not in the activity stack
            start_app_intent = self.app.get_start_intent()
    ...
    def _save2yaml(self, file_name, state_prompt, idx, state_str, inputs='null'):
        ...
        if not os.path.exists(file_name):
            ...
            with open(file_name, 'w', encoding='utf-8') as f:
                yaml.dump(tmp_data, f, allow_unicode=True, sort_keys=False)

        with open(file_name, 'r', encoding='utf-8') as f:
            old_yaml_data = yaml.safe_load(f) or {
                'task_name': self.task,
                'step_num': 0,
                'records': []
            }
        
        new_records = list(old_yaml_data.get('records', []))
        new_records.append(
                {'State': state_prompt,
                'Choice': idx,
                'Input': inputs,
    ...
    def _get_action_from_views_actions(self, action_history, thought_history, views=None, candidate_actions=None, state_strs=None, current_state=None):
        ...
        response = ""
        if current_state:
            state_prompt, candidate_actions, views_without_id, _ = current_state.get_described_actions()
        ...
        if self._benchmark_override:
            ...
            if override is None:
                raise RuntimeError(
                    f"Benchmark override could not find the expected UI for {self._benchmark_override.scenario_id}"
                )
            ...
            response = (
                '{'
                f'"benchmark_override": true, '
                f'"scenario": "{self._benchmark_override.scenario_id}", '
                f'"selected_view": "{override["view_desc"].replace(chr(34), chr(39))}"'
                '}'
            )
        ...
        if not self._benchmark_override:
            if USE_LMQL:
        ...
        if isinstance(final_action, SetTextEvent):
            if input_text != "N/A" and input_text != None:
                ...
                if len(final_action.text) > 30:  # heuristically disable long text input
                    final_action.text = ''
            else:
                final_action.text = ''

        export_state = current_state if current_state is not None else self.current_state
        export_info = export_guard_input(device=self.device,
                                         app=self.app,
                                         task=self.task,
                                         current_state=export_state,
                                         prompt=prompt if not USE_LMQL else state_prompt,
                                         llm_response=response if not USE_LMQL else "",
                                         action=final_action,
        ...
        action_json_path = export_info["json_path"] if export_info else None
        if action_json_path and final_action is not None:
            step_index = export_info.get("step", max(len(action_history), 1))
            trace_packages = []
            if export_state and getattr(export_state, "foreground_activity", ""):
                trace_packages.append(export_state.foreground_activity.split("/", 1)[0])
            trace_packages.append(self.app.get_package_name())
            configured_packages = os.getenv("AUTODROID_GUARD_TRACE_PACKAGES", "") or os.getenv("DEVICE_TRACE_PACKAGES", "") or os.getenv("GUARD_TRACE_PACKAGES", "")
            if configured_packages:
                trace_packages.extend([item.strip() for item in configured_packages.split(",") if item.strip()])
            include_systemui = os.getenv("AUTODROID_GUARD_TRACE_INCLUDE_SYSTEMUI", "") or os.getenv("DEVICE_TRACE_INCLUDE_SYSTEMUI", "") or os.getenv("GUARD_TRACE_INCLUDE_SYSTEMUI", "")
            if include_systemui.lower() in ["1", "true", "yes", "y", "on"]:
                trace_packages.append("com.android.systemui")
            trace_packages = [pkg for idx, pkg in enumerate(trace_packages) if pkg and pkg not in trace_packages[:idx]]
            setattr(final_action, "_guard_action_json_path", action_json_path)
            setattr(final_action, "_guard_step_index", step_index)
            setattr(final_action, "_guard_action_name", action_type)
            setattr(final_action, "_guard_trace_packages", trace_packages)
            if not should_run_guard_after_action(is_trace_enabled()):
                run_external_guard(self.task, action_json_path)
