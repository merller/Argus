...
from .guard_adapter import get_guard_method_name, get_trace_dir_name, is_trace_enabled
...
class Device(object):
    ...
    def __init__(self, device_serial=None, is_emulator=False, output_dir=None,
        ...
        if device_serial is not None and not str(device_serial).strip():
            device_serial = None
    ...
    def send_event(self, event):
        ...
        action_json_path = getattr(event, "_guard_action_json_path", None)
        if trace_enabled and action_json_path and self.adb is not None:
            trace_dir = os.path.join(
                self.output_dir or ".",
                get_trace_dir_name(),
                get_guard_method_name(),
            )
            trace_name = "step_%03d_%s" % (
                int(getattr(event, "_guard_step_index", 0)),
                str(getattr(event, "_guard_action_name", "action"))
            )
            trace_packages = getattr(event, "_guard_trace_packages", [])
            self.adb.set_trace_context(trace_dir, trace_name, trace_packages)
        try:
            event.send(self)
        finally:
            if trace_enabled and action_json_path and self.adb is not None:
                self.adb.stop_trace()
                self.adb.clear_trace_context()
    ...
    def get_top_activity_name(self):
        ...
        patterns = [
            re.compile(r'\* Hist #\d+: ActivityRecord{[^ ]+ [^ ]+ ([^ ]+) t(\d+)}'),
            re.compile(r'mResumedActivity: ActivityRecord{[^ ]+ [^ ]+ ([^ }]+)'),
            re.compile(r'ResumedActivity: ActivityRecord{[^ ]+ [^ ]+ ([^ }]+)'),
            re.compile(r'topResumedActivity=ActivityRecord{[^ ]+ [^ ]+ ([^ }]+)'),
            re.compile(r'ACTIVITY ([A-Za-z0-9_.$]+)/([A-Za-z0-9_.$]+)'),
        ]
        for pattern in patterns:
            m = pattern.search(r)
            if not m:
                continue
            if pattern.pattern.startswith("ACTIVITY "):
                return m.group(1) + "/" + m.group(2)
            return m.group(1)

        window_dump = self.adb.shell("dumpsys window windows")
        window_patterns = [
            re.compile(r'mCurrentFocus=Window{[^ ]+ [^ ]+ ([A-Za-z0-9_.$]+/[A-Za-z0-9_.$]+)}'),
            re.compile(r'mFocusedApp=.*ActivityRecord{[^ ]+ [^ ]+ ([A-Za-z0-9_.$]+/[A-Za-z0-9_.$]+)'),
        ]
    ...
    def take_screenshot(self):
        ...
        keep_temp_history = _env_flag("AUTODROID_KEEP_TEMP_SCREENSHOTS", False)
        if keep_temp_history:
            from datetime import datetime
            tag = datetime.now().strftime("%Y-%m-%d_%H%M%S")
            image_basename = "screen_%s" % tag
        else:
            image_basename = "screen_current"

        if self.adapters[self.minicap] and self.minicap.last_screen:
            # minicap use jpg format
            local_image_path = os.path.join(local_image_dir, "%s.jpg" % image_basename)
            ...
            local_image_path = os.path.join(local_image_dir, "%s.png" % image_basename)
            remote_image_path = "/sdcard/%s.png" % image_basename
