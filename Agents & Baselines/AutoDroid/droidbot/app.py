...
class App(object):
    ...
    def get_start_intent(self):
        ...
        if self.get_main_activity():
            component = package_name + "/%s" % self.get_main_activity()
    ...
    def get_start_with_profiling_intent(self, trace_file, sampling=None):
        ...
        if sampling is not None:
            return Intent(
                prefix="start --start-profiler %s --sampling %d" % (trace_file, sampling),
                component=component,
                suffix="" if component else package_name,
            )
        else:
            return Intent(
                prefix="start --start-profiler %s" % trace_file,
                component=component,
                suffix="" if component else package_name,
            )
