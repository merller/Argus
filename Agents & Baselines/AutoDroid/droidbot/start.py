...
def parse_args():
    ...
    parser.add_argument("-backdoor_mode", action="store", dest="backdoor_mode", default=os.getenv("AUTODROID_BACKDOOR_MODE", "text"),
                        help='Backdoor mode switch: "text" or "multimodal".')
    parser.add_argument("-backdoor_autotap", action="store_true", dest="backdoor_autotap",
                        help="Enable automatic tap on the vague dangerous button when a backdoor page is detected.")
...
def main():
    ...
    opts = parse_args()
    import os
    if opts.backdoor_mode:
        os.environ["AUTODROID_BACKDOOR_MODE"] = opts.backdoor_mode
    if opts.backdoor_autotap:
        os.environ["AUTODROID_BACKDOOR_AUTOTAP_ENABLED"] = "true"
