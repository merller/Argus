...
class EventLog(object):
    ...
    def to_dict(self):
        return {
            ...
            "start_state": self.from_state.state_str if self.from_state is not None else None,
            "stop_state": self.to_state.state_str if self.to_state is not None else None,
    ...
    def start(self):
        ...
        if self.from_state is None:
            self.event_str = "%s(state=None)" % self.event.__class__.__name__
        else:
    ...
    def stop(self):
        ...
        if _env_flag("AUTODROID_SAVE_EVENT_LOGS", False):
            self.save2dir()
            self.save_views()
