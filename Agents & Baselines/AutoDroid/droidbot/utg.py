...
class UTG(object):
    ...
    def add_node(self, state):
        ...
        if state.state_str not in self.G.nodes():
            if _env_flag("AUTODROID_SAVE_STATES", False):
                state.save2dir()
        ...
        if state.foreground_activity and state.foreground_activity.startswith(self.app.package_name):
    ...
    def __output_utg(self):
        ...
        """
        if not self.device.output_dir or not _env_flag("AUTODROID_SAVE_UTG", False):
            return
        ...
        for state_str in self.G.nodes():
            ...
            foreground_activity = state.foreground_activity or ""
            if "/" in foreground_activity:
                package_name, activity_name = foreground_activity.split("/", 1)
            else:
                package_name = self.app.package_name
                activity_name = foreground_activity or "unknown"
