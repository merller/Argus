...
from .guard_adapter import attach_execution_trace, run_external_guard, is_trace_enabled, should_run_guard_after_action
...
class InputManager(object):
    ...
    def add_event(self, event):
        ...
        if action_json_path and trace_enabled and getattr(self.device, "adb", None) is not None:
            trace_info = self.device.adb.get_last_trace()
            if trace_info:
                attach_execution_trace(action_json_path, trace_info)

        if action_json_path and should_run_guard_after_action(trace_enabled):
