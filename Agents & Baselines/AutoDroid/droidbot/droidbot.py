...
from .guard_adapter import is_trace_enabled
...
class DroidBot(object):
    ...
    def start(self):
        ...
        try:
            ...
            if not self.enabled:
                return
            if self.device and self.device.adb and is_trace_enabled():
                self.device.adb.prepare_trace_runtime(app=self.app, packages=[self.app.get_package_name()])

            if os.getenv("AUTODROID_MANUAL_APP_LAUNCH", "").lower() in ["1", "true", "yes", "y", "on"]:
                self._wait_for_manual_app_launch()
    ...
    def _wait_for_manual_app_launch(self):
        wait_interval = float(os.getenv("AUTODROID_MANUAL_APP_WAIT_INTERVAL", "1.0"))
        timeout = float(os.getenv("AUTODROID_MANUAL_APP_WAIT_TIMEOUT", "0"))
        started_at = time.time()
        self.logger.info("Manual launch mode enabled. Open %s on the device, then return here.", self.app.get_package_name())
        while self.enabled:
            try:
                if self.device.is_foreground(self.app):
                    self.logger.info("%s is now in foreground.", self.app.get_package_name())
                    return
                if timeout > 0 and (time.time() - started_at) > timeout:
                    self.logger.info("Manual launch wait timed out, continuing anyway.")
                    return
            except Exception:
                pass
    ...
    def stop(self):
        ...
        if self.droidbox:
            self.droidbox.stop()
        if self.device and self.device.adb:
            try:
                self.device.adb.cleanup_trace_runtime()
            except Exception:
                pass
        if self.device:
