...
from ..guard_trace import GuardTraceManager
try:
...
class ADB(Adapter):
    ...
    def __init__(self, device=None):
        ...
        self.trace_manager = GuardTraceManager(self)
    ...
    def _run_host_cmd(self, cmd):
        return subprocess.run(cmd, stdout=subprocess.PIPE, stderr=subprocess.PIPE, text=True)

    def _run_device_shell(self, shell_command, require_root=False):
        variants = []
        if require_root and os.getenv("AUTODROID_GUARD_TRACE_USE_SU", "true").lower() in ["1", "true", "yes", "y", "on"]:
            variants.append(self.cmd_prefix + ["shell", "su", "0", "sh", "-c", shell_command])
        variants.append(self.cmd_prefix + ["shell", "sh", "-c", shell_command])

        last_result = None
        for cmd in variants:
            result = self._run_host_cmd(cmd)
            last_result = result
    ...
    def _query_package_pids(self, package_name):
        ...
        result = self._run_device_shell("pidof %s" % quote(package_name), require_root=False)
        if result is not None and result.returncode == 0 and result.stdout.strip():
            return [pid for pid in result.stdout.strip().split() if pid.isdigit()]

        result = self._run_device_shell(
            "ps -A -o PID,NAME 2>/dev/null | grep -w %s" % quote(package_name),
            require_root=False,
        )
        ...
        for line in result.stdout.splitlines():
            ...
            if len(fields) >= 2 and fields[-1] == package_name and fields[0].isdigit():
                pids.append(fields[0])
        return pids

    def set_trace_context(self, trace_dir, trace_name, packages=None):
        return self.trace_manager.set_trace_context(trace_dir, trace_name, packages=packages, app=getattr(self.device, "app", None))

    def stop_trace(self):
        return self.trace_manager.stop_trace()

    def clear_trace_context(self):
        self.trace_manager.clear_trace_context()

    def get_last_trace(self):
        return self.trace_manager.get_last_trace()

    def prepare_trace_runtime(self, app=None, packages=None):
        return self.trace_manager.prepare_runtime(app=app, packages=packages)

    def cleanup_trace_runtime(self):
        return self.trace_manager.cleanup_runtime()
