...
def parse_args():
    ...
    parser.add_argument("-task", action="store", dest="task", default=_default_task(),
                        help="the task to execute, in natural language; defaults to $TASK if set")
    ...
    parser.add_argument("--guard-method", action="store", dest="guard_method", default=os.getenv("AUTODROID_GUARD_METHOD_NAME", os.getenv("GUARD_METHOD_NAME", "")),
                        help="Enable a defense: asb, delimiters, temporal_pusv, task_shield, agrail, kairos, agent_sentinel, or agentsight.")
    parser.add_argument("--guard-script", action="store", dest="guard_script", default=os.getenv("AUTODROID_GUARD_SCRIPT_PATH", os.getenv("GUARD_SCRIPT_PATH", "")),
                        help="Optional external guard script. Leave empty to use the unified router for --guard-method.")
    parser.add_argument("--guard-run-external", action="store_true", dest="guard_run_external",
                        help="Run the selected guard after exporting each action.")
    parser.add_argument("--guard-export", action="store_true", dest="guard_export",
                        help="Export guard input JSON without necessarily running a guard.")
...
def main():
    ...
    if not os.path.exists(opts.apk_path):
        ...
        return
    if opts.guard_method:
        os.environ["AUTODROID_GUARD_METHOD_NAME"] = opts.guard_method
    if opts.guard_script:
        os.environ["AUTODROID_GUARD_SCRIPT_PATH"] = opts.guard_script
    if opts.guard_run_external:
        os.environ["AUTODROID_GUARD_RUN_EXTERNAL"] = "true"
        os.environ["AUTODROID_GUARD_EXPORT_ENABLED"] = "true"
    if opts.guard_export:
        os.environ["AUTODROID_GUARD_EXPORT_ENABLED"] = "true"
