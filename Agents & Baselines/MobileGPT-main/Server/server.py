...
class Server:
    ...
    def handle_client(self, client_socket, client_address):
        ...
        guard_adapter = GuardAdapter()
        mobileGPT.guard_adapter = guard_adapter
        app_agent = AppAgent()
        ...
        while True:
            ...
            if not raw_message_type:
                ...
                guard_adapter.complete_pending_action()
                guard_adapter.cleanup_trace_runtime()
                client_socket.close()
                return
            ...
            if message_type == 'L':
                ...
                mobileGPT.target_package = target_package
                guard_adapter.set_instruction_context(instruction, task, target_package, log_directory)
                ...
                current_package = self.__extract_current_package(raw_xml)
                parsed_xml, hierarchy_xml, encoded_xml = screen_parser.encode(raw_xml, screen_count)
                guard_adapter.set_screen_context(
                    screen_index=screen_count,
                    screenshot_path=os.path.join(log_directory, "screenshots", f"{screen_count}.jpg"),
                    raw_xml_path=os.path.join(log_directory, "xmls", f"{screen_count}.xml"),
                    parsed_xml_path=os.path.join(log_directory, "xmls", f"{screen_count}_parsed.xml"),
                    hierarchy_xml_path=os.path.join(log_directory, "xmls", f"{screen_count}_hierarchy_parsed.xml"),
                    encoded_xml_path=os.path.join(log_directory, "xmls", f"{screen_count}_encoded.xml"),
    ...
    @staticmethod
    def __extract_current_package(raw_xml):
