...
static inline JValue Execute(
  ...
  // A continuation root must be sealed from ART on every way out of Execute(),
  // including instrumentation force-pop, exception unwind, and a compiled-code
  // bridge.  ArgusContinuationMethodExit() is a no-op until this exact frame has
  // entered the armed root, so installing the guard before the early returns is
  // both safe and necessary.
  auto argus_method_exit = android::base::make_scope_guard(
      [&shadow_frame]() REQUIRES_SHARED(Locks::mutator_lock_) {
        ArgusContinuationMethodExit(&shadow_frame);
      });
