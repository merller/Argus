...
static ALWAYS_INLINE bool DoInvoke(Thread* self,
  ...
  // The target has now been resolved using the real receiver, but its first
  // instruction has not executed.  This is the continuation decision point.
  if (UNLIKELY(!ArgusContinuationObserveCall(
          sf_method,
          shadow_frame.GetDexPC(),
          called_method,
          static_cast<int>(type)))) {
    result->SetJ(0);
    shadow_frame.SetForcePopFrame(true);
    return true;
  }
