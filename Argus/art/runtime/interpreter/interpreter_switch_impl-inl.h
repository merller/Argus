...
#define HANDLER_ATTRIBUTES ALWAYS_INLINE FLATTEN WARN_UNUSED REQUIRES_SHARED(Locks::mutator_lock_)
  ...
  HANDLER_ATTRIBUTES bool Preamble() {
    ArgusContinuationMethodPreamble(&shadow_frame_, shadow_frame_.GetMethod());
    if (UNLIKELY(ArgusContinuationShouldForceReturn(&shadow_frame_))) {
      shadow_frame_.SetForcePopFrame(true);
    }
  ...
  HANDLER_ATTRIBUTES bool HandleBranch(int32_t offset,
    ...
    if (UNLIKELY(!ArgusContinuationObserveBranch(
            shadow_frame_.GetMethod(), DexPC(), offset, selector_candidate))) {
      shadow_frame_.SetForcePopFrame(true);
      return true;
    }
  ...
  HANDLER_ATTRIBUTES bool HandleIf(bool cond, int32_t offset) {
    return HandleBranch(cond ? offset : Instruction::SizeInCodeUnits(kFormat), true);
  }
...
#pragma clang diagnostic pop
  ...
  HANDLER_ATTRIBUTES bool PACKED_SWITCH() {
    return HandleBranch(DoPackedSwitch(inst_, shadow_frame_, inst_data_), true);
  }

  HANDLER_ATTRIBUTES bool SPARSE_SWITCH() {
    return HandleBranch(DoSparseSwitch(inst_, shadow_frame_, inst_data_), true);
  }
