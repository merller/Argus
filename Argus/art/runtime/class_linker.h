...
class ClassLinker {
 public:
  ...
  // Returns the lifetime-unique registration generation for a loaded DexFile. This is safe for
  // cache identities; a later DexFile mapped at the same address receives a different value.
  bool GetDexRegistrationIndex(Thread* self, const DexFile& dex_file, uint64_t* index) const
      REQUIRES_SHARED(Locks::mutator_lock_)
      REQUIRES(!Locks::dex_lock_);
