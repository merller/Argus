...
bool ClassLinker::GetDexRegistrationIndex(Thread* self,
                                          const DexFile& dex_file,
                                          uint64_t* index) const {
  DCHECK(index != nullptr);
  ReaderMutexLock mu(self, *Locks::dex_lock_);
  auto iterator = dex_caches_.find(&dex_file);
  if (iterator == dex_caches_.end()) {
    return false;
  }
  *index = iterator->second.registration_index;
  return true;
}
