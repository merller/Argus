...
// 统一的核心函数：从 jmethodID 拿到 DexFile
// 新增：一次性返回 checksum + method_index，避免两次调用
extern "C" JNIEXPORT jstring JNICALL
ArtGetDciCodebaseFromListener(JNIEnv* env, jobject listener) {
  LOG(ERROR) << "ArtGetDciCodebaseFromListener: called";  // ART 内部用 LOG
    if (listener == nullptr) return nullptr;

    jclass listenerClass = env->GetObjectClass(listener);
    jmethodID onClick = env->GetMethodID(listenerClass, "onClick", "(Landroid/view/View;)V");
    if (onClick == nullptr) { env->ExceptionClear(); return nullptr; }

    ArtMethod* method = jni::DecodeArtMethod(onClick);
    if (method == nullptr) return nullptr;

    ScopedObjectAccess soa(Thread::Current());
    const DexFile* dex_file = method->GetDexFile();
    if (dex_file == nullptr) return nullptr;

    char buf[16];
    snprintf(buf, sizeof(buf), "%08x", dex_file->GetHeader().checksum_);
    return env->NewStringUTF(buf);
}

extern "C" JNIEXPORT jint JNICALL
ArtGetDciIndexFromListener(JNIEnv* env, jobject listener) {
  LOG(ERROR) << "ArtGetDciIndexFromListener: called";  // ART 内部用 LOG
    if (listener == nullptr) return -1;

    jclass listenerClass = env->GetObjectClass(listener);
    jmethodID onClick = env->GetMethodID(listenerClass, "onClick", "(Landroid/view/View;)V");
    if (onClick == nullptr) { env->ExceptionClear(); return -1; }
    ...
    ScopedObjectAccess soa(Thread::Current());
    const DexFile* dex_file = method->GetDexFile();
    if (dex_file == nullptr) return -1;

    uint32_t idx = method->GetDexMethodIndex();
    return (idx == dex::kDexNoIndex) ? -1 : static_cast<jint>(idx);
}
