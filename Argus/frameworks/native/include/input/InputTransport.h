...
struct InputMessage {
    ...
    union Body {
        struct Key {
            ...
            bool isInjected;
            uint8_t empty1[3];
            uint64_t argusCounter __attribute__((aligned(8)));
            uint64_t argusMac;
        ...
        struct Motion {
            ...
            bool isInjected;
            uint8_t empty2[2]; // 2 bytes to fill the gap after classification and isInjected
            uint64_t argusCounter __attribute__((aligned(8)));
            uint64_t argusMac;
            int32_t edgeFlags;
            nsecs_t downTime __attribute__((aligned(8)));
...
public:
    ...
    status_t publishKeyEvent(uint32_t seq, int32_t eventId, int32_t deviceId, int32_t source,
                             ...
                             bool isInjected = false, uint64_t argusCounter = 0,
                             uint64_t argusMac = 0);
...
public:
    ...
    status_t consume(InputEventFactoryInterface* factory, bool consumeBatches, nsecs_t frameTime,
                     ...
                     bool* outIsInjected = nullptr, uint64_t* outArgusCounter = nullptr,
                     uint64_t* outArgusMac = nullptr);
...
private:
    ...
    status_t consumeBatch(InputEventFactoryInterface* factory, nsecs_t frameTime,
