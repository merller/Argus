...
namespace {
constexpr int32_t ARGUS_SOURCE_PHYSICAL = 1;
constexpr int32_t ARGUS_SOURCE_INJECTED = 2;
constexpr int32_t ARGUS_SOURCE_ACCESSIBILITY = 3;
constexpr int32_t ARGUS_TAMPERED = -1;
constexpr uint64_t ARGUS_REPLAY_WINDOW = 65'536;

struct __attribute__((packed)) ArgusSubjectMacInput {
    uint32_t version;
    int32_t source;
    int32_t eventId;
    int32_t targetPid;
...
static_assert(sizeof(ArgusSubjectMacInput) == 28);
...
InputDispatcher::InputDispatcher(InputDispatcherPolicyInterface& policy,
    ...
    if (RAND_bytes(reinterpret_cast<uint8_t*>(mArgusAgentKey.data()),
                   sizeof(mArgusAgentKey)) != 1 ||
        RAND_bytes(reinterpret_cast<uint8_t*>(mArgusUserKey.data()),
                   sizeof(mArgusUserKey)) != 1) {
        LOG_ALWAYS_FATAL("Argus could not generate process-private SipHash keys");
    }
}
...
uint64_t InputDispatcher::computeArgusSubjectMac(int32_t source, int32_t eventId,
                                                 uint64_t counter, int32_t targetPid,
                                                 int32_t targetUid) const {
    const ArgusSubjectMacInput input{
            .version = 1,
            .source = source,
            .eventId = eventId,
            .targetPid = targetPid,
    ...
    const std::array<uint64_t, 2>* key = nullptr;
    if (source == ARGUS_SOURCE_PHYSICAL) {
        key = &mArgusUserKey;
    } else if (source == ARGUS_SOURCE_INJECTED || source == ARGUS_SOURCE_ACCESSIBILITY) {
        key = &mArgusAgentKey;
    } else {
        return 0;
    }
    return SIPHASH_24(key->data(), reinterpret_cast<const uint8_t*>(&input), sizeof(input));
}

InputDispatcher::ArgusSubjectTag InputDispatcher::issueArgusSubjectTagLocked(
        int32_t source, int32_t eventId, int32_t targetPid, int32_t targetUid) {
    if (targetPid <= 0 || targetUid < 0) {
        return {};
    }
    const uint64_t counter = mArgusNextCounter++;
    return {.counter = counter,
            .mac = computeArgusSubjectMac(source, eventId, counter, targetPid, targetUid)};
}
...
status_t InputDispatcher::publishMotionEvent(Connection& connection,
    ...
    const int32_t argusSource = (motionEntry.policyFlags & POLICY_FLAG_INJECTED)
            ? ARGUS_SOURCE_INJECTED
            : ARGUS_SOURCE_PHYSICAL;
    ArgusSubjectTag argusTag;
    const sp<WindowInfoHandle> windowHandle =
            getWindowHandleLocked(connection.inputChannel->getConnectionToken());
    if (windowHandle != nullptr) {
        const WindowInfo* info = windowHandle->getInfo();
        argusTag = issueArgusSubjectTagLocked(argusSource, dispatchEntry.resolvedEventId,
                                              info->ownerPid, info->ownerUid);
    }
    ...
    return connection.inputPublisher
            .publishMotionEvent(dispatchEntry.seq, dispatchEntry.resolvedEventId,
                                ...
                                motionEntry.pointerCount, motionEntry.pointerProperties,
                                usingCoords,
                                (motionEntry.policyFlags & POLICY_FLAG_INJECTED) != 0,
                                argusTag.counter, argusTag.mac);
}
    ...
    while (connection->status == Connection::Status::NORMAL && !connection->outboundQueue.empty()) {
        ...
        switch (eventEntry.type) {
            case EventEntry::Type::KEY: {
                ...
                const int32_t argusSource = (keyEntry.policyFlags & POLICY_FLAG_INJECTED)
                        ? ARGUS_SOURCE_INJECTED
                        : ARGUS_SOURCE_PHYSICAL;
                ArgusSubjectTag argusTag;
                const sp<WindowInfoHandle> windowHandle =
                ...
                if (windowHandle != nullptr) {
                    const WindowInfo* info = windowHandle->getInfo();
                    argusTag = issueArgusSubjectTagLocked(argusSource,
                                                          dispatchEntry->resolvedEventId,
                                                          info->ownerPid, info->ownerUid);
                }
                if (DEBUG_OUTBOUND_EVENT_DETAILS) {
                ...
                status = connection->inputPublisher
                                 .publishKeyEvent(dispatchEntry->seq,
                                                  ...
                                                  keyEntry.eventTime,
                                                  (keyEntry.policyFlags & POLICY_FLAG_INJECTED) != 0,
                                                  argusTag.counter, argusTag.mac);
                break;
            }
...
int32_t InputDispatcher::verifyArgusSubjectTag(int32_t source, int32_t eventId,
                                               uint64_t counter, uint64_t mac,
                                               int32_t purpose, int32_t callerPid,
                                               int32_t callerUid) {
    std::scoped_lock _l(mLock);
    const bool validSource = source == ARGUS_SOURCE_PHYSICAL ||
            source == ARGUS_SOURCE_INJECTED || source == ARGUS_SOURCE_ACCESSIBILITY;
    const bool validPurpose = purpose > 0 && purpose <= 31;
    const uint64_t floor = mArgusNextCounter > ARGUS_REPLAY_WINDOW
            ? mArgusNextCounter - ARGUS_REPLAY_WINDOW
            : 1;
    if (!validSource || !validPurpose || counter < floor || counter >= mArgusNextCounter ||
        callerPid <= 0 || callerUid < 0) {
        return ARGUS_TAMPERED;
    }

    const uint32_t purposeBit = 1U << (purpose - 1);
    const auto consumed = mArgusConsumedPurposes.find(counter);
    if (consumed != mArgusConsumedPurposes.end() && (consumed->second & purposeBit) != 0) {
        return ARGUS_TAMPERED;
    }

    const uint64_t expected =
            computeArgusSubjectMac(source, eventId, counter, callerPid, callerUid);
    if (CRYPTO_memcmp(&expected, &mac, sizeof(mac)) != 0) {
        return ARGUS_TAMPERED;
    }

    mArgusConsumedPurposes[counter] |= purposeBit;
    if (mArgusConsumedPurposes.size() > 2 * ARGUS_REPLAY_WINDOW) {
        std::erase_if(mArgusConsumedPurposes,
                      [floor](const auto& entry) { return entry.first < floor; });
    }
    return source;
}

std::array<uint64_t, 2> InputDispatcher::mintArgusAgentTag(int32_t source, int32_t eventId,
                                                           int32_t callerPid,
                                                           int32_t callerUid) {
    if (source != ARGUS_SOURCE_INJECTED && source != ARGUS_SOURCE_ACCESSIBILITY) {
        return {0, 0};
    }
    std::scoped_lock _l(mLock);
    const ArgusSubjectTag tag =
            issueArgusSubjectTagLocked(source, eventId, callerPid, callerUid);
    return {tag.counter, tag.mac};
}
