...
public:
    ...
    int32_t verifyArgusSubjectTag(int32_t source, int32_t eventId, uint64_t counter,
                                 uint64_t mac, int32_t purpose, int32_t callerPid,
                                 int32_t callerUid) override;
    std::array<uint64_t, 2> mintArgusAgentTag(int32_t source, int32_t eventId,
                                             int32_t callerPid, int32_t callerUid) override;
...
private:
    ...
    ArgusSubjectTag issueArgusSubjectTagLocked(int32_t source, int32_t eventId,
                                               int32_t targetPid, int32_t targetUid)
            REQUIRES(mLock);
    uint64_t computeArgusSubjectMac(int32_t source, int32_t eventId, uint64_t counter,
                                    int32_t targetPid, int32_t targetUid) const;
    std::array<uint64_t, 2> mArgusAgentKey;
    std::array<uint64_t, 2> mArgusUserKey;
    uint64_t mArgusNextCounter GUARDED_BY(mLock) = 1;
    std::unordered_map<uint64_t, uint32_t> mArgusConsumedPurposes GUARDED_BY(mLock);
    ...
    status_t publishMotionEvent(Connection& connection, DispatchEntry& dispatchEntry)
            REQUIRES(mLock);
