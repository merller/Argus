...
public:
    ...
    /* Verify and one-shot-consume an Argus TLS subject token for a framework purpose. */
    virtual int32_t verifyArgusSubjectTag(int32_t source, int32_t eventId, uint64_t counter,
                                         uint64_t mac, int32_t purpose, int32_t callerPid,
                                         int32_t callerUid) = 0;

    /* Mint an agent-only token for trusted accessibility action entry points. */
    virtual std::array<uint64_t, 2> mintArgusAgentTag(int32_t source, int32_t eventId,
                                                     int32_t callerPid, int32_t callerUid) = 0;
