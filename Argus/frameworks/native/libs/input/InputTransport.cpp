...
void InputMessage::getSanitizedCopy(InputMessage* msg) const {
    ...
    switch(header.type) {
        case InputMessage::Type::KEY: {
            ...
            // bool isInjected
            msg->body.key.isInjected = body.key.isInjected;
            // uint64_t argusCounter
            msg->body.key.argusCounter = body.key.argusCounter;
            // uint64_t argusMac
            msg->body.key.argusMac = body.key.argusMac;
            // int32_t deviceId
        ...
        case InputMessage::Type::MOTION: {
            ...
            // bool isInjected
            msg->body.motion.isInjected = body.motion.isInjected;
            // uint64_t argusCounter
            msg->body.motion.argusCounter = body.motion.argusCounter;
            // uint64_t argusMac
            msg->body.motion.argusMac = body.motion.argusMac;
            // int32_t edgeFlags
...
status_t InputPublisher::publishKeyEvent(uint32_t seq, int32_t eventId, int32_t deviceId,
                                         ...
                                         nsecs_t eventTime, bool isInjected,
                                         uint64_t argusCounter, uint64_t argusMac) {
    if (ATRACE_ENABLED()) {
        std::string message =
    ...
    msg.body.key.isInjected = isInjected;
    msg.body.key.argusCounter = argusCounter;
    msg.body.key.argusMac = argusMac;
...
status_t InputPublisher::publishMotionEvent(
        ...
        const PointerCoords* pointerCoords, bool isInjected, uint64_t argusCounter,
        uint64_t argusMac) {
    if (ATRACE_ENABLED()) {
    ...
    msg.body.motion.isInjected = isInjected;
    msg.body.motion.argusCounter = argusCounter;
    msg.body.motion.argusMac = argusMac;
...
status_t InputConsumer::consume(InputEventFactoryInterface* factory, bool consumeBatches,
                                ...
                                bool* outIsInjected, uint64_t* outArgusCounter,
                                uint64_t* outArgusMac) {
    ...
    *outSeq = 0;
    *outEvent = nullptr;
    if (outIsInjected != nullptr) {
        *outIsInjected = false;
    }
    if (outArgusCounter != nullptr) {
        *outArgusCounter = 0;
    }
    if (outArgusMac != nullptr) {
        *outArgusMac = 0;
    }
    ...
    while (!*outEvent) {
        if (mMsgDeferred) {
            ...
            if (result) {
                ...
                if (consumeBatches || result != WOULD_BLOCK) {
                    result = consumeBatch(factory, frameTime, outSeq, outEvent, outIsInjected,
                                          outArgusCounter, outArgusMac);
                    if (*outEvent) {
                        ALOGD_IF(DEBUG_TRANSPORT_CONSUMER,
        ...
        switch (mMsg.header.type) {
            case InputMessage::Type::KEY: {
                ...
                *outEvent = keyEvent;
                if (outIsInjected != nullptr) {
                    *outIsInjected = mMsg.body.key.isInjected;
                }
                if (outArgusCounter != nullptr) {
                    *outArgusCounter = mMsg.body.key.argusCounter;
                }
                if (outArgusMac != nullptr) {
                    *outArgusMac = mMsg.body.key.argusMac;
                }
                ALOGD_IF(DEBUG_TRANSPORT_CONSUMER,
            ...
            case InputMessage::Type::MOTION: {
                ...
                *outEvent = motionEvent;
                if (outIsInjected != nullptr) {
                    *outIsInjected = mMsg.body.motion.isInjected;
                }
                if (outArgusCounter != nullptr) {
                    *outArgusCounter = mMsg.body.motion.argusCounter;
                }
                if (outArgusMac != nullptr) {
                    *outArgusMac = mMsg.body.motion.argusMac;
                }
...
status_t InputConsumer::consumeBatch(InputEventFactoryInterface* factory, nsecs_t frameTime,
                                     uint32_t* outSeq, InputEvent** outEvent, bool* outIsInjected,
                                     uint64_t* outArgusCounter, uint64_t* outArgusMac) {
    status_t result;
    for (size_t i = mBatches.size(); i > 0; ) {
        ...
        if (frameTime < 0) {
            result = consumeSamples(factory, batch, batch.samples.size(), outSeq, outEvent,
                                    outIsInjected, outArgusCounter, outArgusMac);
        ...
        result = consumeSamples(factory, batch, split + 1, outSeq, outEvent, outIsInjected,
                                outArgusCounter, outArgusMac);
...
status_t InputConsumer::consumeSamples(InputEventFactoryInterface* factory,
        ...
        bool* outIsInjected, uint64_t* outArgusCounter, uint64_t* outArgusMac) {
    MotionEvent* motionEvent = factory->createMotionEvent();
    if (! motionEvent) return NO_MEMORY;

    if (outIsInjected != nullptr) {
        *outIsInjected = batch.samples[0].body.motion.isInjected;
    }
    if (outArgusCounter != nullptr) {
        *outArgusCounter = batch.samples[0].body.motion.argusCounter;
    }
    if (outArgusMac != nullptr) {
        *outArgusMac = batch.samples[0].body.motion.argusMac;
    }
    uint32_t chain = 0;
    for (size_t i = 0; i < count; i++) {
...
bool InputConsumer::canAddSample(const Batch& batch, const InputMessage *msg) {
    ...
    if (head.body.motion.pointerCount != pointerCount
            ...
            || head.body.motion.isInjected != msg->body.motion.isInjected) {
        return false;
    }
    for (size_t i = 0; i < pointerCount; i++) {
