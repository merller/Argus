...
void InputEvent::initialize(int32_t id, int32_t deviceId, uint32_t source, int32_t displayId,
    ...
    initialize(id, deviceId, source, displayId, hmac, 0);
}

void InputEvent::initialize(int32_t id, int32_t deviceId, uint32_t source, int32_t displayId,
                            std::array<uint8_t, 32> hmac, uint32_t policyFlags) {
    mId = id;
    mDeviceId = deviceId;
    mSource = source;
...
void InputEvent::initialize(const InputEvent& from) {
    ...
    mHmac = from.mHmac;
    mPolicyFlags = from.mPolicyFlags;
    mArgusSubjectTag = from.mArgusSubjectTag;
}
...
void KeyEvent::initialize(int32_t id, int32_t deviceId, uint32_t source, int32_t displayId,
                          std::array<uint8_t, 32> hmac, uint32_t policyFlags, int32_t action, int32_t flags,
                          int32_t keyCode, int32_t scanCode, int32_t metaState, int32_t repeatCount,
                          nsecs_t downTime, nsecs_t eventTime) {
    InputEvent::initialize(id, deviceId, source, displayId, hmac, policyFlags);
    mAction = action;
    mFlags = flags;
    mKeyCode = keyCode;
    mScanCode = scanCode;
...
void MotionEvent::initialize(int32_t id, int32_t deviceId, uint32_t source, int32_t displayId,
                             std::array<uint8_t, 32> hmac, uint32_t policyFlags, int32_t action, int32_t actionButton,
...
void MotionEvent::copyFrom(const MotionEvent* other, bool keepHistory) {
    InputEvent::initialize(other->mId, other->mDeviceId, other->mSource, other->mDisplayId,
                           other->mHmac, other->mPolicyFlags);
    setArgusSubjectTag(other->mArgusSubjectTag);
