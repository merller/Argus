...
static jint nativeVerifyArgusSubjectTag(JNIEnv* env, jobject nativeImplObj, jint source,
                                        jint eventId, jlong counter, jlong mac, jint purpose,
                                        jint callerPid, jint callerUid) {
    NativeInputManager* im = getNativeInputManager(env, nativeImplObj);
    return im->getInputManager()->getDispatcher().verifyArgusSubjectTag(
            source, eventId, static_cast<uint64_t>(counter), static_cast<uint64_t>(mac), purpose,
            callerPid, callerUid);
}

static jlongArray nativeMintArgusAgentTag(JNIEnv* env, jobject nativeImplObj, jint source,
                                          jint eventId, jint callerPid, jint callerUid) {
    NativeInputManager* im = getNativeInputManager(env, nativeImplObj);
    const std::array<uint64_t, 2> tag =
            im->getInputManager()->getDispatcher().mintArgusAgentTag(source, eventId, callerPid,
                                                                     callerUid);
    jlongArray result = env->NewLongArray(2);
    if (result == nullptr) {
        return nullptr;
    }
    const jlong values[] = {static_cast<jlong>(tag[0]), static_cast<jlong>(tag[1])};
...
static const JNINativeMethod gInputManagerMethods[] = {
        ...
        {"verifyArgusSubjectTag", "(IIJJIII)I", (void*)nativeVerifyArgusSubjectTag},
        {"mintArgusAgentTag", "(IIII)[J", (void*)nativeMintArgusAgentTag},
