...
public final class ViewRootImpl implements ViewParent,
    ...
    @Nullable
    View findArgusTargetForInputEvent(InputEvent event) {
        if (!(event instanceof MotionEvent) || mView == null) {
            return null;
        }
        final MotionEvent motion = (MotionEvent) event;
        final int action = motion.getActionMasked();
        if (action != MotionEvent.ACTION_DOWN && action != MotionEvent.ACTION_UP) {
            return null;
        }
        return ArgusViewTargetResolver.findClickableTarget(mView, motion.getX(), motion.getY());
    }

    @Nullable
    ArgusClickDci buildArgusClickDciForTarget(@Nullable View target) {
        if (target == null) {
            return null;
        }
        if (!ArgusOverrideGuard.hasRequiredSuperDispatch(target)) {
            return ArgusClickDci.blockedByOverride(target);
        }
        final View.OnClickListener listener = target.getArgusOnClickListenerForDci();
        return listener != null ? View.buildClickDci(target.getContext(), target.getId(), listener)
                : null;
    }
    ...
    final class WindowInputEventReceiver extends InputEventReceiver {
        ...
        public void onInputEvent(InputEvent event) {
            if (ArgusInputEventGate.getInstance().shouldDropWindowInputEvent(
                    ViewRootImpl.this, event, this)) {
