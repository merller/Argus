...
public final class ViewRootImpl implements ViewParent,
    ...
    abstract class InputStage {
        ...
        public final void deliver(QueuedInputEvent q) {
            final long previousInputContext =
                    InputEventReceiver.pushInputEventContext(
                            q.mArgusInputSource, q.mArgusInputEventId,
                            q.mArgusCounter, q.mArgusMac);
            try {
    ...
    private static final class QueuedInputEvent {
        ...
        public int mFlags;
        public int mArgusInputSource;
        public int mArgusInputEventId;
        public long mArgusCounter;
        public long mArgusMac;
    ...
    private QueuedInputEvent obtainQueuedInputEvent(InputEvent event,
        ...
        q.mFlags = flags;
        q.mArgusInputSource = InputEventReceiver.getCurrentInputEventSource();
        q.mArgusInputEventId = event.getId();
        q.mArgusCounter = InputEventReceiver.getCurrentArgusCounter();
        q.mArgusMac = InputEventReceiver.getCurrentArgusMac();
        return q;
    }
    ...
    private void recycleQueuedInputEvent(QueuedInputEvent q) {
        q.mEvent = null;
        q.mReceiver = null;
        q.mArgusInputSource = InputEventReceiver.INPUT_EVENT_SOURCE_UNKNOWN;
        q.mArgusInputEventId = -1;
        q.mArgusCounter = 0;
        q.mArgusMac = 0;
    ...
    // sys: UI event 生命周期结束，回收 agent TLS 标记
    // 此时 onClick 等所有 App 回调已执行完毕，ArtMethod::Invoke 不会再读取此标记
    AccessibilityClickMarker.clearMarker();
    ...
    final class WindowInputEventReceiver extends InputEventReceiver {
        ...
        public void onInputEvent(InputEvent event) {
            if (event instanceof MotionEvent) {
                ...
                if (action == MotionEvent.ACTION_DOWN || action == MotionEvent.ACTION_UP) {
                    Log.i("ArgusInput", "receiver eventId=" + event.getId()
                            + " action=" + MotionEvent.actionToString(action)
                            + " source=" + argusInputSourceToString(
                                    InputEventReceiver.getCurrentInputEventSource()));
                }
            }
        ...
        private String argusInputSourceToString(int source) {
            switch (source) {
                case InputEventReceiver.INPUT_EVENT_SOURCE_PHYSICAL:
                    return "physical";
                case InputEventReceiver.INPUT_EVENT_SOURCE_INJECTED:
                    return "injected";
                case InputEventReceiver.INPUT_EVENT_SOURCE_UNKNOWN:
                default:
                    return "unknown";
            }
        }
