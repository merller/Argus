...
public final class AccessibilityInteractionController {
    ...
    private static final int ARGUS_PURPOSE_ACCESSIBILITY_ACTION = 4;
    ...
    private void performAccessibilityActionUiThread(Message message) {
        ...
        long previousInputContext = 0;
        boolean subjectContextPushed = false;
        try {
            final long[] subjectTag = InputEventReceiver.mintArgusAgentTag(
                    InputEventReceiver.INPUT_EVENT_SOURCE_ACCESSIBILITY, interactionId);
            if (subjectTag == null || subjectTag.length != 2
                    || InputEventReceiver.verifyArgusSubject(
                            InputEventReceiver.INPUT_EVENT_SOURCE_ACCESSIBILITY, interactionId,
                            subjectTag[0], subjectTag[1], ARGUS_PURPOSE_ACCESSIBILITY_ACTION)
                            != InputEventReceiver.INPUT_EVENT_SOURCE_ACCESSIBILITY) {
                InputEventReceiver.clearArgusSubjectContext();
                Log.w(LOG_TAG, "Argus rejected unauthenticated accessibility action");
                return;
            }
            previousInputContext = InputEventReceiver.pushInputEventContext(
                    InputEventReceiver.INPUT_EVENT_SOURCE_ACCESSIBILITY, interactionId,
                    subjectTag[0], subjectTag[1]);
            subjectContextPushed = true;
            if (mViewRootImpl.mView == null || mViewRootImpl.mAttachInfo == null ||
            ...
            if (target != null && isShown(target) && isVisibleToAccessibilityService(target)) {
                ...
                if (action == R.id.accessibilityActionClickOnClickableSpan) {
                    ...
                    final String operation = argusOperationForAccessibilityAction(action);
                    final String operationArguments = "virtualNodeId=" + virtualDescendantId
                            + ";action=" + action;
                    AccessibilityNodeProvider provider = target.getAccessibilityNodeProvider();
                    if (provider != null) {
                        final boolean preflightAllowed = ArgusDci.log(target, provider,
                                "performAction",
                                "(IILandroid/os/Bundle;)Z",
                                operation, "pre_effect", interactionId,
                                InputEventReceiver.INPUT_EVENT_SOURCE_ACCESSIBILITY,
                                operationArguments);
                        if (preflightAllowed) {
                            final long continuationToken = ArgusDci.beginContinuation(
                                    target, provider, "performAction",
                                    "(IILandroid/os/Bundle;)Z", operation, interactionId,
                                    InputEventReceiver.INPUT_EVENT_SOURCE_ACCESSIBILITY,
                                    operationArguments);
                            int continuationDecision = 0;
                            try {
                                if (continuationToken >= 0) {
                                    succeeded = provider.performAction(
                                            virtualDescendantId, action, arguments);
                                }
                            } finally {
                                continuationDecision =
                                        ArgusDci.endContinuation(continuationToken);
                            }
                            if (continuationToken < 0 || continuationDecision < 0) {
                                succeeded = false;
                            }
                        }
                    } else if (virtualDescendantId == AccessibilityNodeProvider.HOST_VIEW_ID) {
                        final boolean preflightAllowed = ArgusDci.log(target, target,
                        ...
                        if (preflightAllowed) {
                            final long continuationToken = ArgusDci.beginContinuation(
                                    target, target, "performAccessibilityAction",
                                    "(ILandroid/os/Bundle;)Z", operation, interactionId,
                                    InputEventReceiver.INPUT_EVENT_SOURCE_ACCESSIBILITY,
                                    operationArguments);
                            int continuationDecision = 0;
                            try {
                                if (continuationToken >= 0) {
                                    succeeded = target.performAccessibilityAction(
                                            action, arguments);
                                }
                            } finally {
                                continuationDecision =
            ...
            if (subjectContextPushed) {
                InputEventReceiver.popInputEventContext(previousInputContext);
            }
            try {
                resetAccessibilityFetchFlags();
    ...
    private static String argusOperationForAccessibilityAction(int action) {
    ...
    private boolean handleClickableSpanActionUiThread(
        ...
        if (clickableSpan != null) {
            final String operationArguments = "virtualNodeId=" + virtualDescendantId
                    + ";clickableSpan=true";
            if (!ArgusDci.log(view, clickableSpan, "onClick", "(Landroid/view/View;)V",
                    "click", "pre_effect", -1,
                    InputEventReceiver.INPUT_EVENT_SOURCE_ACCESSIBILITY,
                    operationArguments)) {
                return false;
            }
            final long continuationToken = ArgusDci.beginContinuation(view, clickableSpan,
                    "onClick", "(Landroid/view/View;)V", "click", -1,
                    InputEventReceiver.INPUT_EVENT_SOURCE_ACCESSIBILITY,
                    operationArguments);
            int continuationDecision = 0;
            try {
                ...
                continuationDecision = ArgusDci.endContinuation(continuationToken);
            }
            return continuationDecision >= 0;
        }
        return false;
    }
