...
public abstract class InputEventReceiver {
    ...
    private static final int ARGUS_POLICY_FLAG_INJECTED = 0x01000000;
    ...
    @UnsupportedAppUsage(maxTargetSdk = Build.VERSION_CODES.R, trackingBug = 170729553)
    private void dispatchInputEvent(int seq, InputEvent event, int policyFlags) {
        mSeqMap.put(event.getSequenceNumber(), seq);
        if ((policyFlags & ARGUS_POLICY_FLAG_INJECTED) != 0) {
            AccessibilityClickMarker.writeInjectedEventTag();
        } else if (isAccessibilityGestureEvent(event)) {
            AccessibilityClickMarker.writeAccessibilityGestureTag();
        } else {
            AccessibilityClickMarker.writeUserEventTag();
        }
        onInputEvent(event);
    }

    private static boolean isAccessibilityGestureEvent(InputEvent event) {
        if (event instanceof MotionEvent) {
            return (((MotionEvent) event).getFlags()
                    & MotionEvent.FLAG_IS_ACCESSIBILITY_EVENT) != 0;
        }
        if (event instanceof KeyEvent) {
            return (((KeyEvent) event).getFlags()
                    & KeyEvent.FLAG_IS_ACCESSIBILITY_EVENT) != 0;
        }
        return false;
    }
