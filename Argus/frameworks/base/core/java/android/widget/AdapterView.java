...
public abstract class AdapterView<T extends Adapter> extends ViewGroup {
    ...
    private boolean mArgusSkipNextItemClickPreflight;
    ...
    public boolean performItemClick(View view, int position, long id) {
        final boolean preflightAlreadyMeasured = mArgusSkipNextItemClickPreflight;
        mArgusSkipNextItemClickPreflight = false;
        final boolean result;
        final OnItemClickListener listener = mOnItemClickListener;
        if (listener != null) {
            final View target = view != null ? view : this;
            final int inputEventId = ArgusDci.currentInputEventId();
            final int inputSource = ArgusDci.currentInputSource();
            final String arguments = "parentId=" + getId() + ";position=" + position
                    + ";itemId=" + id;
            if (!preflightAlreadyMeasured) {
                if (!ArgusDci.log(target, listener, "onItemClick",
                        "(Landroid/widget/AdapterView;Landroid/view/View;IJ)V", "item_click",
                        "pre_effect", inputEventId, inputSource, arguments)) {
                    return false;
                }
            }
            final long continuationToken = ArgusDci.beginContinuation(target, listener,
                    "onItemClick",
                    "(Landroid/widget/AdapterView;Landroid/view/View;IJ)V", "item_click",
                    inputEventId, inputSource, arguments);
    ...
    /** Marks the immediate {@code super.performItemClick()} continuation as already measured. */
    final void markArgusItemClickPreflight() {
        mArgusSkipNextItemClickPreflight = true;
    }
    ...
    private void fireOnSelected() {
        ...
        if (selection >= 0) {
            ...
            final long itemId = getAdapter().getItemId(selection);
            ArgusDci.log(v != null ? v : this, mOnItemSelectedListener, "onItemSelected",
                    "(Landroid/widget/AdapterView;Landroid/view/View;IJ)V", "item_select",
                    "callback", ArgusDci.currentInputEventId(),
                    ArgusDci.currentInputSource(),
                    "parentId=" + getId() + ";position=" + selection + ";itemId=" + itemId);
            mOnItemSelectedListener.onItemSelected(this, v, selection,
                    itemId);
        } else {
            ArgusDci.log(this, mOnItemSelectedListener, "onNothingSelected",
                    "(Landroid/widget/AdapterView;)V", "item_select", "callback",
                    ArgusDci.currentInputEventId(), ArgusDci.currentInputSource(),
                    "parentId=" + getId() + ";position=-1");
