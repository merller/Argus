...
status_t NativeInputEventReceiver::consumeEvents(JNIEnv* env,
    ...
    for (;;) {
        ...
        status_t status = mInputConsumer.consume(&mInputEventFactory,
                consumeBatches, frameTime, &seq, &inputEvent, &isInjected,
                &argusCounter, &argusMac);
        if (status != OK && status != WOULD_BLOCK) {
        ...
        if (!skipCallbacks) {
            ...
            if (inputEventObj) {
                ...
                env->CallVoidMethod(receiverObj.get(),
                        ...
                        jboolean(isInjected), static_cast<jlong>(argusCounter),
                        static_cast<jlong>(argusMac));
                if (env->ExceptionCheck()) {
...
int register_android_view_InputEventReceiver(JNIEnv* env) {
    ...
    gInputEventReceiverClassInfo.dispatchInputEvent = GetMethodIDOrDie(env,
            ...
            "dispatchInputEvent", "(ILandroid/view/InputEvent;ZJJ)V");
    gInputEventReceiverClassInfo.onFocusEvent =
